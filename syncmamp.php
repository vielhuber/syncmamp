...
class SyncMAMP
{
	...
	public static function backup($project)
	{
		...
		if( !is_dir($config->source."/".$project) ) { die('folder missing'); }
		if( file_exists( $config->target."/".$project.".zip" ) ) {
			echo "unlinking ".$config->target."/".$project.".zip\n";
			unlink( $config->target."/".$project.".zip" );
		}
		echo "executing zip -r ".$config->target."/".$project.".zip ".$config->source."/".$project."/\n";
		...
		if($return > 0) { unlink( $config->target."/".$project.".mysql" ); }
		...
		if($return > 0) { unlink( $config->target."/".$project.".pgsql" ); }
	...
	{
		$config = json_decode(file_get_contents('config.json'));
		if( !file_exists( $config->target."/".$project.".zip" ) ) { die('missing file'); }
		if( is_dir($config->source."/".$project) ) {
			echo "removing dir ".$config->source."/".$project."\n";
			shell_exec("rm -rf ".$config->source."/".$project);
		}
		echo "executing unzip ".$config->target."/".$project.".zip -d ".$config->source."/".$project."/\n";
